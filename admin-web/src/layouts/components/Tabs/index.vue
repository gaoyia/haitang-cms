    ...
    >
      ...
      >
        ...
        <div class="layout-tabs-bar__label" :class="{ 'tab-label-inner': tabsStyle === 'card' }">
          <KoiGlobalIcon
            v-if="hasIconDisplay(item.icon)"
            :name="item.icon"
            size="16"
            class="layout-tabs-bar__icon"
          />
...
import { hasIconDisplay } from "@/utils/iconDisplay.ts";
...
const tabsNavHeight = computed(() => (tabsStyle.value === "google" ? "32px" : "42px"));
...
.layout-tabs-bar__item.is-active .layout-tabs-bar__close:hover {
  background-color: color-mix(in srgb, var(--layout-main-bg, var(--el-bg-color)) 88%, var(--el-color-primary));
}
...
/** 谷歌 */
.layout-tabs--google .layout-tabs-bar__scroll {
  :deep(.koi-scroll-nav) {
...
.layout-tabs--google .layout-tabs-bar__item {
  ...
  border-radius: var(--layout-main-radius, 8px) var(--layout-main-radius, 8px) 0 0;
  height: 32px;
  line-height: 32px;
  ...
  &:hover:not(.is-active) {
    background-color: color-mix(in srgb, var(--el-fill-color-light) 70%, transparent);
  ...
  &.is-active {
    color: var(--el-color-primary);
    background: var(--layout-main-bg, var(--el-color-primary-light-9));
    ...
    &::after {
      ...
      content: "";
      border-radius: 50%;
      box-shadow: 0 0 0 30px var(--layout-main-bg, var(--el-color-primary-light-9));
    }
