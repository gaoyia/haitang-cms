<template>
  <el-dialog
    v-model="visible"
    :title="isEdit ? t('menu.menu.manage.groupEdit') : t('menu.menu.manage.groupCreate')"
    width="520px"
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="88px">
      ...
      <el-form-item :label="t('menu.menu.manage.groupCode')" prop="code">
        <el-input
          v-model="form.code"
          :disabled="isEdit"
          :placeholder="t('menu.menu.manage.groupCodePh')"
        />
      </el-form-item>
...
const isEdit = computed(() => props.editGroup !== null && !props.editGroup.readonly);
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const res = isEdit.value
        ? await updateMenuGroupApi(props.editGroup!.id, payload)
        : await createMenuGroupApi(payload);
      if (res.code === 0) {
