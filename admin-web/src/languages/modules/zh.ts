export default {
  ...
  menu: {
    ...
    content: {
      ...
      post: {
        manage: {
          ...
          categoryTemplateHint: "当前分类详情模板：{template}",
          categoryTemplateDefault: "默认",
          categoryTemplateGallery: "相册",
          categoryTemplateRecruitment: "招聘",
          categoryTemplateAbout: "关于我们",
          status: "状态",
          draft: "草稿",
          published: "已发布",
          scheduled: "定时发布",
          pinned: "置顶",
          pinnedYes: "置顶",
          pinnedHint: "置顶文章排在列表前部；同组内按展示时间排序",
          displayTime: "展示时间",
          displayTimePh: "留空则使用服务器当前时间",
          displayTimeHint: "留空保存时将使用服务器当前时间",
          publishTime: "发布时间",
          publishTimePh: "留空且已发布时立即发布",
          publishTimeHint: "设定未来时间可定时发布；留空且状态为已发布时将立即发布",
          routePath: "路径",
          publicUrl: "公开链接",
          ...
          tagExtract: "标签提取",
          tagExtractLlm: "大模型提取",
          tagExtractLlmPending: "大模型提取功能尚未接入",
          tagExtractEmpty: "请先填写正文后再提取",
          tagExtractNoResult: "未能从当前文案中提取到合适的关键词",
          tagExtractSuccess: "已添加 {count} 个标签",
          tagExtractFail: "结巴分词加载失败，请稍后重试",
          description: "摘要",
          descriptionPh: "文章摘要",
          content: "正文",
          contentPh: "支持 Markdown 语法，可使用预览与全屏编辑",
          sectionMeta: "基本信息",
          sectionRecruitmentMeta: "招聘扩展信息",
          recruitmentSalary: "薪资",
          recruitmentSalaryPh: "如 15K-25K",
          recruitmentLocation: "地点",
          recruitmentLocationPh: "如 北京 / 远程",
          recruitmentEmploymentType: "用工类型",
          recruitmentEmploymentTypePh: "如 全职 / 实习",
          recruitmentDepartment: "部门",
          recruitmentDepartmentPh: "如 研发",
          sectionAboutMeta: "关于我们扩展信息",
          aboutHighlight: "亮点",
          aboutHighlightPh: "如 结构化优雅",
          aboutFounded: "成立时间",
          aboutFoundedPh: "如 2024",
          aboutLocation: "所在地",
          ...
          assetsNeedSave: "新建时可先选择封面与附件，保存文章后将自动关联",
          pendingAssetsHint: "已选资源尚未关联，请保存文章以完成关联",
          pendingAssetSaved: "资源已选中，保存文章后将自动关联",
          assetAlreadyLinked: "该资源已关联到本文",
          cover: "封面图",
          coverHint: "全语言共用封面，仅支持图片，最多 {max} 张；拖拽左上角手柄可调整排序",
          coverDrag: "拖拽排序",
          coverLimitReached: "已达封面图上限（{max} 张）",
          attachments: "附件",
          attachmentsHint: "可添加多个附件，支持图片、视频、压缩包与 Office 文档等；拖拽左侧手柄可调整排序",
          attachmentsDrag: "拖拽排序",
          selectCover: "选择封面",
          selectAttachment: "选择附件",
          pickerCoverTitle: "选择封面图",
          pickerAttachmentTitle: "选择附件",
          removeOnly: "仅从本文移除",
          removeAndPurge: "移除并删除文件",
          removeCoverConfirm: "移除封面？",
          removeAttachmentConfirm: "移除附件「{name}」？",
          removeAttachmentsConfirm: "移除所选 {count} 个附件？",
          attachmentsSelectedCount: "已选 {count} 项",
        },
      },
      category: {
        title: "分类管理",
        subtitle: "维护文章分类，支持多语言、SEO 路径与列表/详情模板",
        previewLang: "预览语言",
        name: "名称",
        description: "描述",
        sort: "排序",
    ...
    assets: {
      ...
      pickerConfirm: "确认选择",
      pickerConfirmCount: "确认选择 ({count})",
      pickerSelectedCount: "已选 {count} 项",
      pickerSelectMultiHint: "可多选，再次点击取消；点确认批量添加",
      pickerAlreadyLinked: "已关联",
      pickerOpen: "选择或上传图片",
      pickerReplace: "更换图片",
      pickerPurposeHint: "列表与上传均仅限当前用途",
      pickerUploadHint: "上传后自动关联并留在弹窗，可继续上传；也可在资源库中选中后点确认",
    },
    banner: {
      auth: "轮播图",
      groups: "轮播图组",
      ...
      adminTitleHint: "仅管理端列表展示，首页 Hero 文案请在下方按语言配置",
      linkHint: "整张轮播图点击跳转（可选）；Hero 按钮链接在下方单独配置",
      heroSection: "首页 Hero 文案",
      heroSectionHint: "按语言配置角标、标题、描述、标签与按钮；按钮须填写文字与链接地址",
      heroBadge: "角标",
      heroBadgePh: "如：结构化优雅",
      heroTitle: "主标题",
      heroTitlePh: "如：让数字内容如{emOpen}「一缕晨光」{emClose}闪耀荧幕🎉",
      heroTitleHint: "用 em 标签包裹「」引号内的词句即可高亮，格式：让…如{emOpen}「关键词」{emClose}…",
      heroDesc: "描述",
      heroDescPh: "Hero 区域副文案",
      heroTags: "标签",
      heroTagPh: "输入后回车或点添加",
      heroTagAdd: "添加",
      heroTagsHint: "展示为标签组，可添加多项",
      heroActions: "按钮",
      heroActionLabelPh: "按钮文字",
      heroActionUrlPh: "链接地址（站内路径或 https://）",
      heroActionPrimary: "主要",
      heroActionSecondary: "次要",
      heroActionAdd: "添加按钮",
      heroActionsHint: "每个按钮包含显示文字与跳转链接，可配置多个",
    },
    friendLink: {
      auth: "友情链接",
      title: "名称",
      titlePh: "站点名称，用于图片 alt",
      titleRequired: "请输入名称",
      url: "链接",
      urlPh: "https://example.com",
      urlRequired: "请输入链接",
      image: "友链图",
      imageHint: "请从资源库选择 purpose 为「友链」的图片，或先上传友链图",
      imageRequired: "请选择友链图片",
      pickerTitle: "选择友链图片",
      sort: "排序",
      create: "新建友链",
      edit: "编辑友链",
      listEmpty: "暂无友链",
      deleteConfirm: "确认删除友链「{name}」？",
      manage: {
        title: "友情链接",
        subtitle: "管理公开站页脚展示的友情链接，支持排序与启用状态",
        dragSort: "拖拽排序",
      },
    },
    menu: {
      ...
      manage: {
        ...
        itemPath: "路径",
        itemPathRequired: "默认语言路径不能为空",
        itemPathPhAuto: "/about",
        autoLocalePrefix: "自动添加语言前缀",
        autoLocalePrefixHint: "开启后只需填写 /about 等路径，保存时会自动补全 /zh-cn、/en-us 等前缀",
        itemDeleteConfirm: "确认删除菜单「{name}」？子菜单将一并删除",
        dragSort: "拖拽排序",
        treeDragHint: "拖拽菜单项可调整顺序；拖到节点上可设为子菜单，拖到前后可调整同级顺序（类似 VS Code 文件树）",
      },
    },
    dict: {
      auth: "字典管理",
