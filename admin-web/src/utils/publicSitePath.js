...
function buildLangPrefixPattern(locales) {
    const part = locales.map(escapeRegex).join("|");
    return new RegExp(`^/(${part})(/|$)`, "i");
}
/** 开发服务器上应跳过、交给 Vite 自身处理的路径 */
export function shouldSkipPublicRedirect(pathname) {
 ...
 */
export function isPublicSitePath(pathname, localesRaw) {
...
/** 开发环境：跳转到后端公开站同一地址 */
export function redirectToPublicServer(fullPath) {
...
export function isPublicSitePathForDev(pathname, localesRaw) {
    const path = pathname.split("?")[0] || "/";
    if (shouldSkipPublicRedirect(path)) {
        return false;
    }
